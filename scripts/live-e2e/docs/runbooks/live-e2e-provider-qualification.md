...
All four cells must pass on one AOR commit and one pinned target commit.
OpenCode and Qwen remain available as extended provider paths, but do not block
the W66 qualification result. Historical pre-S20 evidence is diagnostic-only.
...
The resulting set uses the W66 qualification schema and embeds the four
...
`scripts/live-e2e/fixtures/evidence/w40-s04/provider-qualification-matrix.sample.json`.
