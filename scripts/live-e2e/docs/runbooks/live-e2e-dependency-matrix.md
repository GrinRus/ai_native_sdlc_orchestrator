...
| `ky` bounded full-journey profiles | `node >=22`, `npm`, `git`, authenticated provider CLI (`codex` for `openai-primary`, Claude Code for `anthropic-primary`, `opencode` for `open-code-primary`, `qwen` for `qwen-primary`; Qwen host auth may require the `ANTHROPIC_AUTH_TOKEN -> ANTHROPIC_API_KEY` adapter alias) | profile-bounded `npm install --prefer-online --no-audit --no-fund`; when the frozen target has no root npm lockfile, the runner adds an auditable `--before` cutoff at the next UTC day after the target commit; npm cache is isolated by stable target dependency snapshot hash; diagnostic full-suite installs Playwright, runs `npm test` with only the timing-sensitive `totalTimeout` case excluded, then runs that exact case alone | small: `npx xo`<br>`npm run build`<br>`npx ava test/headers.ts`; medium regress: `npx xo`<br>`npm run build`<br>`npx ava test/hooks.ts`<br>`npx ava test/retry.ts --match='*shouldRetry*'`; release: `npx xo`<br>`npm run build`<br>`npx ava test/hooks.ts`; large/xlarge governance: `npx xo`<br>`npm run build`<br>`npx ava test/main.ts test/hooks.ts --match='!*totalTimeout bounds a never-ending successful response body*'`<br>`npx ava test/retry.ts --match='*shouldRetry*'`; the timing-sensitive case remains isolated diagnostic evidence, and the xlarge complete split suite remains manual/overnight diagnostic evidence rather than quick acceptance | npm packages + Playwright browser binaries for diagnostic full-suite + provider auth/config | missing provider CLI/auth, permission/readiness denial, provider timeout, target setup/verification blocker with `failure_owner=target_repository`, AOR runner blocker with `failure_owner=aor`, or no-upstream-write violation |
...
- the committed W25-S03 fixture lives at `scripts/production-readiness/fixtures/w25-s03-production-proof.json`; it is sanitized proof evidence for the `ky.regress.small.openai` production cell only, and proof integrity rejects mock-backed `full_code_changing_runtime` claims.
