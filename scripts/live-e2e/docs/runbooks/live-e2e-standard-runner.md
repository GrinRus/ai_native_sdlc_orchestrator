...
For medium+ `mission_class=product-change` profiles, the controller also writes
a `live-e2e-step-quality-assessment-request` after each accepted operator
decision and stops until a linked evaluator-authored
`live-e2e-step-quality-assessment-report` is accepted. Small
`mission_class=flow-regression` canaries may use lightweight flow-health
assessment. Product-change reports must cite the source request, evaluator input
refs, output ref, rubric version, non-empty dimension summaries/findings, and
public evidence refs before continuation.
...
review, delivery, and final run summary changed-path evidence must all preserve
the same target-root lineage. If the root is missing, not a git checkout, or
source reports observe meaningful changed paths that the final summary loses,
medium+ product-change proof fails closed instead of counting as product-quality
acceptance.
...
The human catalog stays in `scripts/live-e2e/docs/runbooks/live-e2e-target-catalog.md`; machine-readable matrix definitions live under:
...
- Codex full-bypass: the selected route/profile model and reasoning effort are rendered by the Codex adapter, followed by `--ask-for-approval never` and `codex exec --ignore-user-config --ignore-rules` with the configured workspace sandbox.
- Codex restricted: the same adapter-owned selection mapping plus `codex exec --ignore-user-config --ignore-rules` without the approval bypass.
...
Runtime model and reasoning selection is provider-neutral. A route or private
profile may set `model` and `reasoning_effort`; the adapter validates and maps
those values at spawn. When omitted, the installed CLI owns the native default.
The Codex Luna/high rehearsal profile demonstrates this path without changing
...
`scripts/live-e2e/docs/runbooks/live-e2e-provider-qualification.md`.
...
`npm install --prefer-online --no-audit --no-fund`; for a target without a root
lockfile, the runner adds `--before=<target-commit-cutoff>` where the cutoff is
the next UTC day after the frozen target commit. The runner assigns the target
commit, commit timestamp, effective setup commands, and dependency manifests a
stable SHA-256 snapshot hash and isolates npm's cache under that hash. This
prevents stale host metadata and post-target semver releases from crossing
runs while allowing a resumed run to reuse the same dependency namespace.
Lockfile-backed installs and commands with an explicit `--before` remain
unchanged. Playwright browser installation remains diagnostic evidence from
...
`scripts/production-readiness/fixtures/w25-s03-production-proof.json`. It is a sanitized derivative of the real
...
- A blocked proof run is not product-quality acceptance, even when contract
  validation and local regression checks pass. Medium+ acceptance requires
  run-health pass, accepted linked step-quality reports for every observed
  step, final-code-quality all-pass, non-empty mission-relevant changed paths,
  and passing target verification or a documented non-AOR/non-product-quality
  blocker.
...
Canonical fixtures:
- `scripts/live-e2e/fixtures/evidence/w21-s07/installed-user-guided-proof.sample.json`
- `scripts/live-e2e/fixtures/evidence/w21-s07/installed-user-guided-app-smoke.sample.json`
- `scripts/live-e2e/fixtures/evidence/w21-s07/installed-user-guided-blocked-readiness.sample.json`
...
- Durable onboarding, intake, next-action, run, review, review-decision,
  delivery, release, learning, and web smoke artifacts, including Task
  Workspace, **New task**, **Prepare task**, project-switcher, and retired
  renderer absence markers.
...
Canonical fixture:
- `scripts/production-readiness/fixtures/w25-s03-production-proof.json`
...
- `scripts/live-e2e/fixtures/evidence/w35-s05/silent-provider-ux-proof.sample.json`
...
- Targeted regression coverage lives in
  `scripts/live-e2e/test/live-e2e-proof-runner.test.mjs` and asserts the W35 fixture
  preserves fail-closed operator evidence, readable artifact summaries, and
  decision-helper required refs.
- `scripts/live-e2e/fixtures/evidence/w35-s05/live-attempts-summary.sample.json`
