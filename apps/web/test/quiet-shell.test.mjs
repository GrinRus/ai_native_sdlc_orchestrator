...
test("adaptive shell keeps lifecycle and presentation semantics explicit", () => {
  ...
  for (const marker of ["Context", "Current lifecycle stage", "View lifecycle stage", "Cockpit", "Attention", "Journey", "Evidence", "Technical context"]) assert.ok(source.includes(marker) || marker === "Context");
  assert.match(source, /aria-current/u); assert.doesNotMatch(source, /sessionStorage|localStorage/u);
  assert.match(css, /grid-template-columns: repeat\(7, minmax\(0, 1fr\)\)/u); assert.match(css, /--aor-control-touch/u); assert.match(css, /prefers-reduced-motion/u);
});
