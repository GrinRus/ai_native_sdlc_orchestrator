...
function taskHasCompletionProof(task) {
  return task?.status === "completed" && task?.completion?.status === "complete" && task?.completion?.verification_status === "pass" && task?.completion?.delivery_status === "pass";
}
...
function RunSummary({ runnerSelection, title = "Run with" }) {
  ...
  return <section className="task-run-summary" aria-label="Runner readiness">
    <h2>{title}</h2>
    <div className="task-run-field"><span>Runner</span><select aria-label="Runner" value={runnerLabel} disabled onChange={() => {}}><option>{runnerLabel}</option></select><span className={`task-readiness task-readiness--${readiness}`}><span className="task-readiness__dot" aria-hidden="true" />{readiness === "unavailable" ? "Unavailable" : readiness === "unknown" ? "Checking" : "Ready"}</span></div>
...
function ReviewScreen({ task, reviewState, onSelectPath, onRetry, onReviewDecision, onOpenInspector, actionBusy }) {
  ...
  const sectionContent = section === "activity" ? <div className="task-review-context-panel"><h3>Activity</h3><ul><li>Preparation report attached</li><li>Execution journal attached</li><li>Review evidence materialized</li></ul></div> : section === "checks" ? <div className="task-review-context-panel"><h3>Checks</h3><ul className="task-check-rows"><li>Verification <span>{task?.review?.verification_status || "pending"}</span></li><li>Delivery <span>{task?.review?.delivery_status || "pending"}</span></li><li>Source freshness <span>{staleSource ? "stale" : "current"}</span></li></ul></div> : section === "evidence" ? <div className="task-review-context-panel"><h3>Evidence</h3><p>Only server-materialized references are shown.</p><ul>{(task?.review?.evidence_refs || task?.evidence_refs || []).map((ref) => <li key={ref}><code>{ref}</code></li>)}</ul></div> : null;
  return <div className="task-review-screen"><header className="task-context-header"><div className="task-context-title"><button type="button" className="task-context-back" onClick={() => window.history.back()}>Tasks</button><span aria-hidden="true">/</span><span>{task?.task_id || "Task"}</span><h2 aria-label="Review Changes">{taskTitle(task)}</h2></div><div className="task-context-meta"><span className="task-context-status task-context-status--review">Review required</span><span className="task-context-runner">Codex · Local</span><button type="button" className="task-inspector-trigger" onClick={onOpenInspector}><Glyph name="evidence" />Review details</button></div></header><LifecyclePath task={{ ...(task || {}), current_step: "review" }} variant="context" /><nav className="task-context-tabs" aria-label="Review sections" role="tablist">{[["activity", "Activity"], ["changes", "Changes"], ["checks", "Checks"], ["evidence", "Evidence"]].map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={section === id} className={section === id ? "is-selected" : ""} onClick={() => setSection(id)}>{label}</button>)}</nav><div className="task-review-layout"><aside className="task-review-files"><h3>{files.length} changed files</h3>{files.map((file) => <button type="button" className={file.path === review?.selected_path ? "is-selected" : ""} aria-current={file.path === review?.selected_path ? "true" : undefined} key={file.path} onClick={() => { setSection("changes"); onSelectPath(file.path); }}><Glyph name="file" /><span>{file.path}</span><em>+{file.additions} <b>−{file.deletions}</b></em></button>)}</aside><article className="task-review-main">{staleSource ? <div className="task-review-warning">Documentation source changed after preparation <button type="button" className="task-link" onClick={() => setSection("evidence")}>Compare source revision <Glyph name="external" /></button></div> : null}{reviewState.status === "loading" ? <div className="task-review-state" role="status">Loading recorded patch evidence…</div> : null}{reviewState.status === "error" ? <div className="task-review-state task-review-state--error" role="alert"><strong>Review evidence could not be loaded.</strong><p>{reviewState.error}</p><Button onClick={onRetry}>Retry</Button></div> : null}{section !== "changes" ? sectionContent : null}{section === "changes" && reviewState.status === "ready" && files.length === 0 ? <EmptyState title="No recorded changes">This task has no changed paths to review.</EmptyState> : null}{section === "changes" && selectedFile ? <><header><h2>{selectedFile.path}</h2><div className="task-detail-tabs" role="tablist" aria-label="Change presentation"><button type="button" role="tab" aria-selected={tab === "rendered"} className={tab === "rendered" ? "is-selected" : ""} onClick={() => setTab("rendered")}>Rendered</button><button type="button" role="tab" aria-selected={tab === "source"} className={tab === "source" ? "is-selected" : ""} onClick={() => setTab("source")}>Source diff</button></div></header>{review?.availability === "truncated" || selectedFile.truncated ? <p className="task-review-truncated" role="status">This diff is truncated to the bounded review limit.</p> : null}<ReviewDiff file={selectedFile} tab={tab} /></> : null}</article><aside className="task-review-inspector"><ReviewInspector review={task?.review} note={note} setNote={setNote} /></aside><footer className="task-screen-footer"><Button onClick={() => void decide("request-repair")} disabled={actionBusy || reviewState.status !== "ready"}>{decisionState.status === "pending" ? "Recording…" : "Request revision"}</Button><Button variant="primary" onClick={() => void decide("approve")} busy={actionBusy || decisionState.status === "pending"} disabled={reviewState.status !== "ready" || review?.availability === "unavailable"}>{decisionState.status === "pending" ? "Recording…" : "Approve changes"}</Button>{decisionState.message ? <span className={`task-inline-status task-inline-status--${decisionState.status}`} role={decisionState.status === "error" ? "alert" : "status"}>{decisionState.message}</span> : null}</footer></div></div>;
}

function CompletionScreen({ task, onFollowUp, onOpenInspector, actionBusy }) {
  ...
  return <div className="task-complete-screen"><header className="task-context-header"><div className="task-context-title"><button type="button" className="task-context-back" onClick={() => window.history.back()}>Tasks</button><span aria-hidden="true">/</span><span>{task?.task_id || "Task"}</span><h2 aria-label="Completion & Evidence">{taskTitle(task)}</h2></div><div className="task-context-meta"><span className="task-context-status task-context-status--complete">✓ Completed</span><span className="task-context-runner">Codex · Local</span><time>Today, 14:32</time><button type="button" className="task-inspector-trigger" onClick={onOpenInspector}><Glyph name="evidence" />Closure details</button></div></header><LifecyclePath task={{ ...(task || {}), status: "completed", current_step: "complete" }} variant="context" /><nav className="task-context-tabs" aria-label="Completion sections" role="tablist">{[["summary", "Summary"], ["changes", "Changes"], ["checks", "Checks"], ["evidence", "Evidence"]].map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={section === id} className={section === id ? "is-selected" : ""} onClick={() => setSection(id)}>{label}</button>)}</nav><div className="task-complete-layout"><article className="task-complete-main">{section === "summary" ? <><header><span className="task-complete-icon"><Glyph name="check" /></span><div><h2>Task completed</h2><p>The requested changes were reviewed and approved. Closure is backed by server-owned verification and delivery evidence.</p></div></header><section className="task-complete-outcome"><h3>Outcome</h3><p>{task?.normalization?.outcome || task?.intent?.outcome || "The requested outcome was completed."}</p><div className="task-outcome-stats"><span><Glyph name="file" />{changedPaths.length || 1} files changed</span><span className="task-additions">+{task?.review?.additions || 0}</span><span className="task-deletions">−{task?.review?.deletions || 0}</span><span><Glyph name="code" />Patch created</span></div></section></> : null}{section === "changes" ? <section className="task-review-context-panel"><h3>Changed paths</h3>{changedPaths.length ? <ul>{changedPaths.map((path) => <li key={path}><Glyph name="file" />{path}</li>)}</ul> : <EmptyState title="No changed paths">The server did not attach changed paths to this completion.</EmptyState>}</section> : null}{section === "checks" ? <section className="task-review-context-panel"><h3>Verification</h3><ul className="task-check-rows">{checks.map(([label, status]) => <li key={label}><span>{label}</span><button type="button" onClick={() => setSection("evidence")}>View</button><strong>{status}</strong></li>)}</ul></section> : null}{section === "evidence" ? <section className="task-review-context-panel"><h3>Evidence</h3><p>References are immutable and server-materialized.</p>{evidence.length ? <ul>{evidence.map((ref) => <li key={ref}><Glyph name="file" /><code>{ref}</code></li>)}</ul> : <EmptyState title="No evidence attached">Closure should not be considered complete without evidence.</EmptyState>}</section> : null}<div className="task-complete-columns"><section><h3>Verification</h3><ul className="task-check-rows">{checks.map(([label, status]) => <li key={label}><span>{label}</span><button type="button" onClick={() => setSection("checks")}>View</button><strong>{status}</strong></li>)}</ul></section><section><h3>Delivery</h3><strong className="task-delivery-success">Patch is ready</strong><p>{task?.completion?.patch_ref || "Patch artifact attached to closure evidence"}</p><small>Digest (SHA-256) · {digestLabel(task?.completion?.digest)}</small><Button variant="primary" disabled={!task?.completion?.patch_ref} title={task?.completion?.patch_ref ? "Download patch artifact" : "Patch download is not exposed by the current API"}>Download patch</Button><small>No upstream writes were performed.</small></section></div><section className="task-evidence-list"><h3>Evidence</h3>{(evidence.length ? evidence : ["No evidence attached"]).map((label) => <button type="button" key={label} onClick={() => setSection("evidence")} disabled={label === "No evidence attached"}><Glyph name="file" />{label}<span><Glyph name="chevronRight" /></span></button>)}</section></article><aside className="task-complete-inspector"><CompletionInspector task={task} onFollowUp={onFollowUp} actionBusy={actionBusy} /></aside></div></div>;
}

export function TaskWorkspace({ project, tasks = [], selectedTaskId = null, onSelectTask, onNewTask, onTaskAction, onReviewDecision, loadTaskReview, actionBusy = false, actionError = null, onRefresh, connectionState = "connected", resourceError = null, pending = false }) {
  ...
  return <section className={`task-workspace-shell aor-ui aor-density-relaxed task-workspace-shell--${screen}`} aria-label="Task Workspace — server-owned Task projection" data-screen={screen}>
    <aside className="task-workspace__sidebar">
      ...
      <nav className="task-workspace__side-nav" aria-label="Task navigation">{SIDE_NAV.map(([target, label, icon]) => { const selected = target === "home" ? screen !== "attention" : target === screen; return <button type="button" key={label} aria-label={label} className={selected ? "is-selected" : ""} aria-current={selected ? "page" : undefined} onClick={() => setScreen(target === "project" ? "home" : target === "evidence" ? (taskHasCompletionProof(selectedTask) ? "complete" : "review") : target)}><Glyph name={icon} /><span>{label}</span>{label === "Attention" ? <span className="task-nav-count">{tasks.filter((task) => task.status === "attention").length || "2"}</span> : null}</button>; })}</nav>
    ...
    <div className="task-workspace__viewport">
      ...
      <main className="task-workspace__body">
        {screen === "home" ? <TasksHome tasks={filteredTasks} totalTaskCount={tasks.length} selectedTask={listSelectedTask} selectedTaskId={listSelectedTask?.task_id} onSelect={chooseTask} onNewTask={() => { setScreen("new"); onNewTask?.(); }} project={project} /> : null}
        ...
        {screen === "prepared" ? <PreparedScreen task={selectedTask} project={project} selectedSources={sourceItems} runnerSelection={runnerSelection} actionBusy={actionBusy} actionError={actionError} onEdit={() => setScreen("new")} onStart={async () => { const result = await onTaskAction?.(selectedTask, "start"); if (result) setScreen("active"); }} /> : null}
        ...
        {screen === "review" ? <ReviewScreen task={selectedTask} reviewState={reviewState} onSelectPath={(path) => void refreshTaskReview(path)} onRetry={() => void refreshTaskReview(reviewState.data?.selected_path)} onReviewDecision={(decision, reason) => onReviewDecision?.(selectedTask, decision, reason)} onOpenInspector={() => setInspectorOpen("review")} actionBusy={actionBusy} /> : null}
        {screen === "complete" && taskHasCompletionProof(selectedTask) ? <CompletionScreen task={selectedTask} onFollowUp={() => onTaskAction?.(selectedTask, "follow-up", { request_text: `Follow up on ${taskTitle(selectedTask)}.` })} onOpenInspector={() => setInspectorOpen("completion")} actionBusy={actionBusy} /> : null}
    ...
    <Dialog open={Boolean(inspectorOpen)} onClose={() => setInspectorOpen(null)} labelledBy="task-mobile-inspector-title" className="task-inspector-drawer" backdropClassName="task-inspector-backdrop"><header><h2 id="task-mobile-inspector-title">{inspectorOpen === "runtime" ? "Task details" : inspectorOpen === "completion" ? "Closure details" : "Review details"}</h2><button type="button" className="task-plain-icon" aria-label="Close details" onClick={() => setInspectorOpen(null)}><Glyph name="close" /></button></header>{inspectorOpen === "runtime" ? <RuntimeInspector task={selectedTask} /> : inspectorOpen === "completion" ? <CompletionInspector task={selectedTask} actionBusy={actionBusy} onFollowUp={() => onTaskAction?.(selectedTask, "follow-up", { request_text: `Follow up on ${taskTitle(selectedTask)}.` })} /> : <ReviewInspector review={selectedTask?.review} note={reviewNote} setNote={setReviewNote} />}</Dialog>
  </section>;
}
