...
export function TaskWorkspace({ project, tasks = [], selectedTaskId = null, onSelectTask, onNewTask, onTaskAction, actionBusy = false, actionError = null, onRefresh, connectionState = "connected", resourceError = null, pending = false }) {
  ...
  return <section className="task-workspace aor-ui" aria-label="Task Workspace">
    ...
    {screen === "active" ? <Card><h2>Active Task Workspace</h2><p>{taskTitle(selectedTask)}</p><div className="task-workspace__tabs"><button type="button" className="selected">Activity</button><button type="button" onClick={() => setScreen("review")}>Changes</button><button type="button">Checks</button><button type="button" onClick={() => setScreen("attention")}>Evidence</button></div><p>{selectedTask?.current_step_label || "Waiting for runtime activity"}</p><div className="task-workspace__actions"><Button onClick={() => onTaskAction?.(selectedTask, "pause")} disabled={actionBusy}>Pause</Button><Button variant="secondary" onClick={() => onTaskAction?.(selectedTask, "cancel")} disabled={actionBusy}>Cancel</Button><Button variant="secondary" onClick={() => onTaskAction?.(selectedTask, "retry")} disabled={actionBusy}>Request retry</Button><Button variant="secondary" onClick={() => setScreen("review")}>Review</Button></div></Card> : null}
    {screen === "attention" ? <Card><h2>Attention</h2><p>Resolve the recorded blocker or durable operator request before continuing.</p><ul><li>{selectedTask?.blocker_count ?? 0} blockers</li><li>{selectedTask?.attention_count ?? 0} attention items</li></ul><label htmlFor="task-operator-request">Bounded operator request</label><textarea id="task-operator-request" rows="4" value={operatorRequestText} onChange={(event) => setOperatorRequestText(event.target.value)} placeholder="Describe the bounded recovery or question." /><div className="task-workspace__actions"><Button variant="primary" onClick={() => onTaskAction?.(selectedTask, "request", { request_text: operatorRequestText.trim() || "Inspect the recorded Task blocker." })} disabled={actionBusy}>Create durable request</Button><Button variant="secondary" onClick={() => setScreen("active")}>Return to task</Button></div></Card> : null}
