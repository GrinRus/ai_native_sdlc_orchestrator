...
function TruthfulCompletionScreen({ task, onFollowUp, onOpenInspector, onBackToTasks, actionBusy }) {
  ...
  const panel = section === "summary"
    ? <section className="task-completion-summary" role="tabpanel"><header><span className="task-complete-icon"><Glyph name="check" /></span><div><h2>Task completed</h2><p>Closure is backed by server-owned verification and delivery evidence.</p></div></header><section className="task-complete-outcome"><h3>Outcome</h3><p>{task?.normalization?.outcome || task?.intent?.outcome || "The requested outcome was completed."}</p><div className="task-outcome-stats"><span><Glyph name="file" />{changedPaths.length} files changed</span><span><Glyph name="code" />{deliveryRef ? "Delivery evidence attached" : "Delivery reference pending"}</span></div></section></section>
    : section === "changes"
      ? <section className="task-review-context-panel" role="tabpanel"><h3>Changed paths</h3>{changedPaths.length ? <ul>{changedPaths.map((path) => <li key={path}><Glyph name="file" />{path}</li>)}</ul> : <EmptyState title="No changed paths">The server did not attach changed paths to this completion.</EmptyState>}</section>
      : section === "checks"
        ? <section className="task-review-context-panel" role="tabpanel"><h3>Verification</h3><ul className="task-check-rows">{checks.map(([label, status]) => <CheckRow key={label} label={label} status={status} />)}</ul></section>
        : <section className="task-review-context-panel" role="tabpanel"><h3>Evidence</h3><p>References are immutable and server-materialized.</p>{evidence.length ? <ul>{evidence.map((ref) => <li key={ref}><Glyph name="file" /><code>{ref}</code></li>)}</ul> : <EmptyState title="No evidence attached">Closure should not be considered complete without evidence.</EmptyState>}</section>;
...
export function TaskWorkspace({ project, tasks = [], selectedTaskId = null, onSelectTask, onNewTask, onCreateTask, onTaskAction, onReviewDecision, loadTaskReview, actionBusy = false, actionError = null, onRefresh, onOpenProject, connectionState = "connected", resourceError = null }) {
  ...
  const runnerSelection = selectedTask?.runner_selection ?? { route_id: null, readiness: "unknown", recovery_action: "Wait for the server to publish runner readiness." };
  ...
  return <section className={`task-workspace-shell aor-ui aor-density-relaxed task-workspace-shell--${screen}${navCollapsed ? " task-workspace-shell--nav-collapsed" : ""}`} aria-label="Task Workspace — server-owned Task projection" data-screen={screen}>
    ...
    <div className="task-workspace__viewport">
      ...
      <header className="task-workspace__topbar">
        ...
        <div className="task-workspace__top-actions">{["home", "active", "attention", "prepared"].includes(screen) ? <label className="task-search" htmlFor="task-search-input"><Glyph name="search" /><input id="task-search-input" name="task-search" aria-label="Search tasks" placeholder="Search tasks" value={query} onChange={(event) => setQuery(event.target.value)} /><kbd>⌘K</kbd></label> : null}{["home", "active", "attention", "prepared"].includes(screen) ? <select id="task-filter" name="task-filter" aria-label="Filter tasks" className="task-filter" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">All tasks</option><option value="attention">Open</option><option value="active">Active</option><option value="ready">Ready</option><option value="completed">Completed</option></select> : null}{screen === "home" ? <label className="task-group-select" htmlFor="task-group"><select id="task-group" name="task-group" aria-label="Group tasks by" value="status" onChange={() => {}}><option value="status">Group by status</option></select><Glyph name="chevronDown" /></label> : null}<span className="task-live-badge"><Glyph name="signal" />Live</span><span className="task-safe-badge"><Glyph name="shield" />Patch only</span><button type="button" className="task-top-icon task-top-icon--utility" aria-label="Help"><Glyph name="help" /></button><button type="button" className="task-top-icon task-top-icon--utility" aria-label="Settings"><Glyph name="settings" /></button><Button variant="primary" onClick={startNewTask}><Glyph name="plus" />New task<kbd aria-hidden="true">N</kbd></Button><button type="button" className="task-top-icon task-top-icon--keyboard" aria-label="Open command palette" onClick={() => document.querySelector('[aria-label="Search tasks"]')?.focus()}><kbd>⌘</kbd></button></div>
      </header>
