...
function ActiveScreen({ task, project, operatorRequests, operatorRequestText, setOperatorRequestText, onTaskAction, onResumeOperatorRequest, actionBusy, onReview, onOpenInspector }) {
...
function AttentionScreen({ tasks, selectedTask, operatorRequests, operatorRequestText, setOperatorRequestText, onSelect, onTaskAction, onResumeOperatorRequest, actionBusy }) {
...
export function TaskWorkspace({ project, tasks = [], operatorRequests = [], selectedTaskId = null, pendingPreparation = false, pendingSubmissionId = null, onStopFollowingPreparation, onSelectTask, onNewTask, onCreateTask, onTaskAction, onResumeOperatorRequest, onSelectRunner, onCheckPreparationRunner, onCheckExecutionRunner, onInitializeRunnerProfile, executionProfile = null, onReviewDecision, loadTaskReview, actionBusy = false, actionError = null, onRefresh, onOpenProject, connectionState = "connected", resourceError = null }) {
  ...
  return <section className={`task-workspace-shell aor-ui aor-density-relaxed task-workspace-shell--${screen}${navCollapsed ? " task-workspace-shell--nav-collapsed" : ""}`} aria-label="Task Workspace — server-owned Task projection" data-screen={screen}>
    ...
    <div className="task-workspace__viewport">
      ...
      <main className="task-workspace__body">
        ...
        {screen === "active" ? <ActiveScreen task={selectedTask} project={project} runnerSelection={runnerSelection} operatorRequests={operatorRequests} operatorRequestText={operatorRequestText} setOperatorRequestText={setOperatorRequestText} onTaskAction={onTaskAction} onResumeOperatorRequest={onResumeOperatorRequest} actionBusy={actionBusy} onReview={() => setScreen("review")} onOpenInspector={() => setInspectorOpen("runtime")} /> : null}
        {screen === "attention" ? <AttentionScreen tasks={tasks} selectedTask={selectedTask?.status === "attention" ? selectedTask : tasks.find((task) => task.status === "attention")} operatorRequests={operatorRequests} operatorRequestText={operatorRequestText} setOperatorRequestText={setOperatorRequestText} onSelect={chooseTask} onTaskAction={onTaskAction} onResumeOperatorRequest={onResumeOperatorRequest} actionBusy={actionBusy} /> : null}
