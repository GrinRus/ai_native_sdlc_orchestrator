...
function runnableTaskAction(task) {
  const action = task?.primary_action || {};
  const control = action.operator_control;
  const actionId = typeof action.action_id === "string" ? action.action_id.trim() : "";
  const safeToRunInline = [
    Boolean(actionId),
    action.category === "mutation",
    action.permission === "mutate",
    action.available !== false,
    action.requires_confirmation !== true,
    control?.availability !== "blocked",
    control?.requires_confirmation !== true,
    Object.keys(action.payload || {}).length === 0,
  ].every(Boolean);
  return safeToRunInline ? {
    action_id: actionId,
    label: operatorControlLabel(control, "Run server action"),
  } : null;
}

function TaskPrimaryActionButton({ task, onTaskAction, onComplete, actionBusy, variant = "primary" }) {
  const action = runnableTaskAction(task);
  if (!action) return null;
  return <Button variant={variant} onClick={async () => { await onTaskAction?.(task, action.action_id, {}); onComplete?.(); }} busy={actionBusy} disabled={actionBusy}>{actionBusy ? "Running…" : action.label}</Button>;
}
