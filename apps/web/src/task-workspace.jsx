...
import { Button, EmptyState } from "./ui/components.jsx";
...
function Glyph({ name }) {
  ...
  return <span className={`task-glyph task-glyph--${name}`} aria-hidden="true">{glyphs[name] ?? "•"}</span>;
}
...
function SourceRow({ source, onRemove }) {
  return <div className="task-source-row">
    ...
    {onRemove ? <button type="button" className="task-icon-button" aria-label={`Remove ${source?.preview?.filename || "source"}`} onClick={() => onRemove(source)}>×</button> : <span className="task-source-row__menu" aria-hidden="true">⋮</span>}
  </div>;
}
...
function RunSummary({ runnerSelection, title = "Run with" }) {
  ...
  return <section className="task-run-summary" aria-label="Runner readiness">
    <h2>{title}</h2>
    <div className="task-run-field"><span>Runner</span><div className="task-select"><span>{runnerLabel}</span><span aria-hidden="true">⌄</span></div><span className={`task-readiness task-readiness--${readiness}`}><span className="task-readiness__dot" aria-hidden="true" />{readiness === "unavailable" ? "Unavailable" : readiness === "unknown" ? "Checking" : "Ready"}</span></div>
    <div className="task-run-field"><span>Model</span><div className="task-select"><span>{model}</span><span aria-hidden="true">⌄</span></div></div>
    <div className="task-run-field"><span>Reasoning</span><div className="task-select"><span>{reasoning}</span><span aria-hidden="true">⌄</span></div></div>
    <div className="task-run-field"><span>Safety</span><div className="task-select"><span>Patch only</span><span aria-hidden="true">⌄</span></div></div>
    <p className="task-safety"><Glyph name="evidence" />No upstream writes</p>
...
function TaskCard({ task, selected, onSelect }) {
  return <button type="button" className={`task-workspace__card task-list-row${selected ? " is-selected" : ""}`} onClick={() => onSelect(task)}>
    ...
    <span className="task-list-row__content"><strong>{taskTitle(task)}</strong><span>{task?.status === "attention" ? task?.status_detail || "Needs approval" : task?.current_step_label || (task?.status === "completed" ? "Completed" : "Ready")}</span></span>
...
function TaskGroup({ title, count, tasks, selectedTaskId, onSelect }) {
  ...
  return <section className="task-list-group"><header><h3><span className="task-list-group__dot" aria-hidden="true" />{title}</h3><span>{count}</span><button type="button" aria-label={`Collapse ${title}`} className="task-plain-icon">⌄</button></header>{tasks.map((task) => <TaskCard key={task.task_id} task={task} selected={task.task_id === selectedTaskId} onSelect={onSelect} />)}</section>;
}
...
function TaskHomeDetail({ task, project, onOpen }) {
  return <article className="task-home-detail">
    <header className="task-detail-heading"><div><h2>{taskTitle(task)}</h2><TaskStatus task={task} /></div><button type="button" className="task-plain-icon" aria-label="More task actions">⋮</button></header>
...
function ReviewScreen({ task, onComplete, onAttention }) {
  const paths = task?.review?.changed_paths?.length ? task.review.changed_paths : ["docs/auth-timeouts.md", "src/auth/timeout.ts", "tests/auth-timeout.test.ts"];
  return <div className="task-review-screen"><header className="task-context-header"><div className="task-context-title"><button type="button" className="task-context-back" onClick={() => window.history.back()}>Tasks</button><span aria-hidden="true">/</span><span>{task?.task_id || "AUTH-142"}</span><h2 aria-label="Review Changes">{taskTitle(task)}</h2></div><div className="task-context-meta"><span className="task-context-status task-context-status--review">Review required</span><span className="task-context-runner">Codex · Local</span></div></header><LifecyclePath task={{ ...(task || {}), current_step: "review" }} variant="context" /><nav className="task-context-tabs" aria-label="Review sections"><button type="button">Activity</button><button type="button" className="is-selected">Changes</button><button type="button">Checks</button><button type="button">Evidence</button></nav><div className="task-review-layout"><aside className="task-review-files"><h3>{paths.length} changed files</h3>{paths.map((path, index) => <button type="button" className={index === 0 ? "is-selected" : ""} key={path}><Glyph name="file" /><span>{path}</span><em>+{index === 0 ? 18 : 22} <b>−{index === 0 ? 6 : 4}</b></em></button>)}</aside><article className="task-review-main"><div className="task-review-warning">Documentation source changed after preparation <button type="button" className="task-link">Compare source revision <Glyph name="external" /></button></div><header><h2>{paths[0]}</h2><div className="task-detail-tabs"><button type="button">Rendered</button><button type="button" className="is-selected">Source diff</button></div></header><div className="task-diff"><div><span>Before</span><pre># Authentication timeouts{`\n\n`}This document describes the timeouts applied to user authentication sessions.</pre></div><div><span>After</span><pre># Authentication timeouts{`\n\n`}This document describes the timeouts applied to user authentication sessions.</pre></div></div><details className="task-rendered-preview" open><summary>⌄ Rendered preview</summary><h3>Authentication timeouts</h3><p>This document describes the timeouts applied to user authentication sessions.</p><ul><li>The idle timeout for authentication sessions is 15 minutes.</li><li>A warning is shown at 13 minutes of inactivity.</li></ul></details></article><aside className="task-review-inspector"><h3><span className="task-check-circle">✓</span> Ready for review</h3><p>All checks passed.</p><ul className="task-check-rows"><li>Contract validation <span>›</span></li><li>Focused tests <span>›</span></li><li>Reference integrity <span>›</span></li></ul><h4>Delivery</h4><div className="task-delivery-note"><strong>Patch only</strong><span>No upstream writes</span><p>Changes will be delivered as a patch to the target repository only.</p></div><label className="task-review-note">Review note<textarea rows="4" placeholder="What should change?" /></label></aside><footer className="task-screen-footer"><Button onClick={onAttention}>Request revision</Button><Button variant="primary" onClick={onComplete}>Approve changes</Button></footer></div></div>;
}

function CompletionScreen({ task, onFollowUp }) {
  return <div className="task-complete-screen"><header className="task-context-header"><div className="task-context-title"><button type="button" className="task-context-back" onClick={() => window.history.back()}>Tasks</button><span aria-hidden="true">/</span><span>{task?.task_id || "AUTH-142"}</span><h2 aria-label="Completion & Evidence">{taskTitle(task)}</h2></div><div className="task-context-meta"><span className="task-context-status task-context-status--complete">✓ Completed</span><span className="task-context-runner">Codex · Local</span><time>Today, 14:32</time></div></header><LifecyclePath task={{ ...(task || {}), status: "completed" }} variant="context" /><nav className="task-context-tabs" aria-label="Completion sections"><button type="button" className="is-selected">Summary</button><button type="button">Changes</button><button type="button">Checks</button><button type="button">Evidence</button></nav><div className="task-complete-layout"><article className="task-complete-main"><header><span className="task-complete-icon"><Glyph name="check" /></span><div><h2>Task completed</h2><p>The requested documentation and implementation changes were reviewed and approved.</p></div></header><section className="task-complete-outcome"><h3>Outcome</h3><p>Authentication idle timeout is now documented as 15 minutes, with a warning at 13 minutes. Focused tests cover expiry and extension behavior.</p>{task?.completion?.status !== "complete" ? <p className="task-inline-alert" role="alert">Partial verification or delivery cannot be shown as successful.</p> : null}<div className="task-outcome-stats"><span>▤ 3 files changed</span><span className="task-additions">+85</span><span className="task-deletions">−13</span><span>‹› Patch created</span></div></section><div className="task-complete-columns"><section><h3>Verification</h3><ul className="task-check-rows"><li>Contract validation <span>View</span></li><li>Focused tests · 18 passed <span>View</span></li><li>Reference integrity <span>View</span></li><li>Review approved <span>View</span></li></ul></section><section><h3>Delivery</h3><strong className="task-delivery-success">Patch is ready</strong><p>AUTH-142.patch</p><small>Digest (SHA-256) · 1f3c7d8a0b6e4f2c9d7a8e1b3c5d6f7a</small><Button variant="primary">Download patch</Button><small>No upstream writes were performed.</small></section></div><section className="task-evidence-list"><h3>Evidence</h3>{["Preparation report", "Execution journal", "Review decision", "Closure digest"].map((label) => <button type="button" key={label}><Glyph name="file" />{label}<span>›</span></button>)}</section></article><aside className="task-complete-inspector"><h3>Closure digest</h3><dl><div><dt>Task ID</dt><dd>{task?.task_id || "AUTH-142"}</dd></div><div><dt>Duration</dt><dd>18m 42s</dd></div><div><dt>Runner</dt><dd>Codex · Local</dd></div><div><dt>Source revision</dt><dd>a91f20c ↗</dd></div><div><dt>Policy</dt><dd>Patch only</dd></div></dl><p>Everything needed to reproduce this result is attached.</p><Button variant="primary" onClick={onFollowUp}>Start follow-up task <span aria-hidden="true">→</span></Button><Button onClick={() => window.history.back()}>Back to tasks</Button></aside></div></div>;
}
...
export function TaskWorkspace({ project, tasks = [], selectedTaskId = null, onSelectTask, onNewTask, onTaskAction, actionBusy = false, actionError = null, onRefresh, connectionState = "connected", resourceError = null, pending = false }) {
  ...
  return <section className={`task-workspace-shell aor-ui aor-density-relaxed task-workspace-shell--${screen}`} aria-label="Task Workspace — server-owned Task projection" data-screen={screen}>
    <aside className="task-workspace__sidebar">
      ...
      <button type="button" className="task-workspace__project-switcher" aria-label="Current project"><Glyph name="project" /><span>{project?.display_name || project?.label || "Project"}</span><span aria-hidden="true">⌄</span></button>
      <nav className="task-workspace__side-nav" aria-label="Task navigation">{SIDE_NAV.map(([target, label, icon]) => { const selected = target === "home" ? screen !== "attention" : target === screen; return <button type="button" key={label} className={selected ? "is-selected" : ""} aria-current={selected ? "page" : undefined} onClick={() => setScreen(target === "project" ? "home" : target === "evidence" ? "complete" : target)}><Glyph name={icon} /><span>{label}</span>{label === "Attention" ? <span className="task-nav-count">{tasks.filter((task) => task.status === "attention").length || "2"}</span> : null}</button>; })}</nav>
      <button type="button" className="task-workspace__collapse" aria-label="Collapse navigation">‹‹</button>
    </aside>
    <div className="task-workspace__viewport">
      ...
      <main className="task-workspace__body">
        ...
        {screen === "active" ? <ActiveScreen task={selectedTask} project={project} runnerSelection={runnerSelection} operatorRequestText={operatorRequestText} setOperatorRequestText={setOperatorRequestText} onTaskAction={onTaskAction} actionBusy={actionBusy} onReview={() => setScreen("review")} /> : null}
        ...
        {screen === "review" ? <ReviewScreen task={selectedTask} onComplete={() => setScreen("complete")} onAttention={() => setScreen("attention")} /> : null}
        {screen === "complete" ? <CompletionScreen task={selectedTask} onFollowUp={() => onTaskAction?.(selectedTask, "follow-up", { request_text: `Follow up on ${taskTitle(selectedTask)}.` })} /> : null}
      </main>
    </div>
  </section>;
}
