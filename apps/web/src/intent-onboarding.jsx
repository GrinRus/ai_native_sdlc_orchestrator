...
export function IntentOnboarding({ projectId, project, busy, setBusy, onProjectConnected, onStarted, onConfirmed, onCancel, initialSubmission }) {
  ...
  const ready = normalization?.status === "prepared" && submission?.status === "prepared" && !dirty;
  ...
  return <section className="mission-builder aor-ui" aria-label="Intent-first task onboarding" data-server-revision={serverRevision ?? undefined}>
    ...
    {submission && normalization ? <><Card><Field label="Task title"><input name="normalization-title" value={normalization.title || ""} onChange={(event) => updateNormalization({ ...normalization, title: event.target.value })} /></Field><Field label="Outcome"><textarea name="normalization-outcome" value={normalization.outcome || ""} onChange={(event) => updateNormalization({ ...normalization, outcome: event.target.value })} /></Field><div className="mission-preview"><div><span>Code</span><strong>{project?.display_name || connectedProjectId}</strong></div><div><span>Safety</span><strong>{normalization.delivery_mode}</strong></div><div><span>Provider</span><strong>{normalization.provider?.adapter_id || "unavailable"}</strong></div></div><Field label="Work type"><select name="normalization-work-type" value={normalization.work_type || "analyze"} onChange={(event) => updateNormalization({ ...normalization, work_type: event.target.value })}><option value="analyze">Analyze</option><option value="explain">Explain</option><option value="review">Review</option><option value="document-change">Document change</option><option value="code-change">Code change</option></select></Field><div className="intent-planned-path"><span>Planned path</span><strong>{normalization.planned_path?.steps?.map((step) => step.label).join(" → ") || "Runtime will derive the path"}</strong></div><Field label="Constraints" helper="One per line."><textarea name="normalization-constraints" value={(normalization.constraints || []).join("\n")} onChange={(event) => updateNormalization({ ...normalization, constraints: lines(event.target.value) })} /></Field><Field label="Acceptance" helper="One check per line."><textarea name="normalization-acceptance" value={(normalization.acceptance || []).join("\n")} onChange={(event) => updateNormalization({ ...normalization, acceptance: lines(event.target.value) })} /></Field><Field label="Expected scope"><textarea name="normalization-scope" value={(normalization.scope || []).join("\n")} onChange={(event) => updateNormalization({ ...normalization, scope: lines(event.target.value) })} /></Field>{normalization.open_questions?.length ? <div><Alert tone="warning">Answer the blocking questions before confirmation.</Alert>{normalization.open_questions.map((question, index) => <Field key={question} label={question}><input name={`normalization-question-${index + 1}`} value={answers[question] || ""} onChange={(event) => setAnswers({ ...answers, [question]: event.target.value })} /></Field>)}<Button onClick={() => action("answer", { answers })} disabled={busy || normalization.open_questions.some((question) => !answers[question]?.trim())}>Save answers</Button></div> : null}{dirty ? <Alert tone="warning">Unsaved changes are not confirmable. Save this revision before creating the Flow.</Alert> : null}{startFailure ? <Alert tone="danger">The Flow was created, but its first action did not start. Retry uses the same Flow.</Alert> : null}</Card><ResponsiveActions><Button onClick={() => action("revise", { normalization })} disabled={busy || !dirty}>Save revision</Button><Button variant="primary" onClick={() => action("confirm")} disabled={busy || !ready || startFailure}>Confirm and create Flow</Button>{startFailure ? <Button variant="primary" onClick={() => action("retry-start")} disabled={busy}>Retry start</Button> : null}<Button onClick={() => action("retry")} disabled={busy || submission.status === "confirmed"}>Retry preparation</Button><Button onClick={() => action("cancel")} disabled={busy || submission.status === "confirmed"}>Cancel task</Button></ResponsiveActions></> : null}
