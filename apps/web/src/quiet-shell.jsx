...
export function QuietShell({ project, flow, stages, currentStage, viewingStage, mode, onStage, onMode, runtimeRoot, version }) {
  const currentStep = stages.find((stage) => stage.id === currentStage);
  const currentStageLabel = currentStep?.label ?? currentStage;
  return <section className="quiet-shell" aria-label="Quiet Cockpit navigation">
    ...
    <nav className="quiet-stage-path" aria-label={`Lifecycle stages. Current stage: ${currentStageLabel}`}>{stages.map((stage) => <button type="button" key={stage.id} aria-current={stage.id === currentStage ? "step" : undefined} data-selected={stage.id === viewingStage} data-state={stage.state ?? "upcoming"} title={stage.reason ?? undefined} onClick={() => onStage(stage.id)}><span>{stage.label}</span></button>)}</nav>
