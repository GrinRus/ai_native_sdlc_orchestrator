...
function AttentionMode({ projection, status, resourceErrors, onResolve, onInspect }) {
  ...
  return <section className="quiet-mode-panel quiet-attention" aria-label="Attention queue">
    ...
    <div className="quiet-attention-groups">{GROUPS.map(([state, label]) => {
      ...
      return <section key={state}><h2>{label} <span>{entries.length}</span></h2><ul>{entries.map((row) => <li key={row.item_id}><button type="button" aria-pressed={selected?.item_id === row.item_id} onClick={() => setSelectedId(row.item_id)}><StatusBadge tone={row.severity}>{row.stage ?? row.source_family}</StatusBadge><strong>{row.title}</strong><span>{row.consequence}</span></button></li>)}</ul></section>;
    })}</div>
    {selected ? <aside className="quiet-attention-detail" aria-label="Selected attention item"><p className="quiet-mode-eyebrow">Selected item</p><h2>{selected.title}</h2><p>{selected.consequence}</p>{selected.transient_read_error ? <Alert tone="warning">This is a current read failure, not durable completion state.</Alert> : null}<label><span>Operator draft</span><textarea name={`attention-draft-${selected.item_id}`} value={drafts[selected.item_id] ?? ""} placeholder="Add context for the decision or recovery action" onChange={(event) => setDrafts((current) => ({ ...current, [selected.item_id]: event.target.value }))}/></label><div className="quiet-mode-actions">{selected.operator_control?.availability === "ready" ? <Button variant="primary" onClick={() => onResolve(selected)}>{selected.operator_control.label}</Button> : null}<Button variant="secondary" onClick={() => onInspect(selected)}>Inspect evidence</Button></div><Disclosure label="Technical source"><code>{selected.source_ref}</code></Disclosure></aside> : null}
    {selected ? <aside className="quiet-attention-context" aria-label="Attention context"><h3>Context</h3><dl><dt>Stage</dt><dd>{selected.stage ?? "Unscoped"}</dd><dt>State</dt><dd>{selected.state ?? "Needs attention"}</dd><dt>Severity</dt><dd>{selected.severity ?? "warning"}</dd><dt>Evidence</dt><dd><code>{selected.source_ref ?? "pending"}</code></dd></dl><h3>Safety</h3><p>No upstream writes are performed by this review surface.</p></aside> : null}
  </section>;
}
...
function EvidenceMode({ graph, trace, onInspect }) {
  ...
  return <section className="quiet-mode-panel" aria-label="Evidence"><header><div><p className="quiet-mode-eyebrow">Durable artifacts</p><h2>Evidence</h2><p>Reports and packets scoped to the selected Flow.</p></div><StatusBadge tone="neutral">{nodes.length} artifacts</StatusBadge></header><div className="quiet-evidence-layout"><div>{nodes.length ? <ul className="quiet-evidence-list">{nodes.map((node) => <li key={node.node_id}><div><strong>{node.label}</strong><span>{node.family} · {node.stage ?? "unscoped"}</span></div><StatusBadge tone={node.status === "fail" ? "danger" : "neutral"}>{node.status ?? "recorded"}</StatusBadge><Button size="compact" variant="secondary" onClick={() => onInspect({ source_ref: node.ref })}>Inspect</Button></li>)}</ul> : <EmptyState title="No flow evidence">Evidence appears after runtime-owned artifacts exist.</EmptyState>}<Disclosure label="Runtime trace"><p>{trace?.trace_items?.length ?? 0} trace events across {trace?.run_ids?.length ?? 0} runs.</p></Disclosure></div><aside className="quiet-evidence-context" aria-label="Evidence summary"><h3>Evidence health</h3><strong className="quiet-evidence-context__value">{nodes.length ? "Recorded" : "Awaiting evidence"}</strong><p>Runtime-owned artifacts remain scoped to this Flow.</p><dl><dt>Artifacts</dt><dd>{nodes.length}</dd><dt>Trace events</dt><dd>{trace?.trace_items?.length ?? 0}</dd><dt>Write-back</dt><dd>No-write</dd></dl><Button variant="primary" onClick={() => onInspect({ source_ref: nodes[0]?.ref ?? "evidence://flow" })}>Inspect evidence graph</Button></aside></div></section>;
}
