...
test.describe.serial("installed local operator console", () => {
  ...
  test("Task Workspace local proof covers server-owned screens and safe Markdown preview", async ({ page }) => {
    ...
    await expect(page.locator(".task-workspace__card").filter({ hasText: "Draft task" }).locator(".task-list-row__content")).toContainText("Draft task");
    await expect(page.locator(".task-workspace__card").filter({ hasText: "Ready task" }).locator(".task-list-row__content")).toContainText("Ready task");
    await expect(page.locator(".task-workspace__card").filter({ hasText: "Completed task" }).locator(".task-list-row__content")).toContainText("Completed task");
    ...
    await page.getByLabel("Upload Markdown").setInputFiles({ name: "safe.md", mimeType: "text/markdown", buffer: Buffer.from("# Safe\n<script>alert('no')</script>") });
    await expect(page.locator(".task-markdown-preview")).toContainText("# Safe");
    await expect(page.locator(".task-markdown-preview")).not.toContainText("alert");
    await page.getByRole("button", { name: /^Add \d+ source/u }).click();
    await expect(page.getByRole("heading", { name: "Prepared Task" })).toBeVisible();
    await page.getByRole("button", { name: "Start task", exact: true }).click();
    await page.getByRole("button", { name: /^Attention(?:\s|$)/u }).click();
