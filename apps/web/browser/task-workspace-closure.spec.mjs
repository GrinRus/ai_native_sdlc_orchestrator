...
test("W70-S08 installed Task Workspace closure covers sources, recovery, review, and immutable completion", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Markdown Sources", exact: true }).click();
  await page.getByLabel("Markdown source kind").selectOption("upload");
  ...
  await expect(page.locator(".task-workspace__source-preview")).toContainText("# Uploaded");
  await expect(page.locator(".task-workspace__source-preview")).not.toContainText("alert");
  await page.getByLabel("Markdown source kind").selectOption("repository");
  ...
  await expect(page.getByText(/No provider process is started/u)).toHaveCount(0);

  await page.getByRole("button", { name: "Tasks Home", exact: true }).click();
  await page.getByRole("button", { name: "Completed task" }).click();
  await page.getByRole("button", { name: "Completion & Evidence", exact: true }).click();
  ...
  const newTaskButton = page.locator(".task-workspace__header").getByRole("button", { name: "New Task" });
