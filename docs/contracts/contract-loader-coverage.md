...
| Execution and quality | `step-result.md` | `step-result` | `examples/reports/step-result*.yaml` | implemented | Includes closed-set enum check for `step_class` plus nested validation for Runtime Harness decisions, interaction audit refs, repair attempts, external runner evidence, and mission-scoped path arrays. |
...
| Operations | `live-run-event.md` | `live-run-event` | `examples/reports/live-run-event*.yaml` | implemented | Closed-set `event_type` validation plus nested `payload.sequence`, interaction status, answer-audit refs, and raw-answer rejection. |
