...
| Execution and quality | `task-action-catalog.md` | `task-action-catalog` | `examples/tasks/task-action-catalog*.yaml` | implemented | Canonical server-owned Task/Flow action ids, permissions, typed payload requirements, dispatch boundaries, and durable readback fields. |
| Execution and quality | `task-projection.md` | `task-projection` | `examples/tasks/task-projection*.yaml` | implemented | Versioned server-owned Task identity, state, prepared execution contract, route separation, and explicit write effects. |
| Core packets and profiles | `task-source-item.md` | `task-source-item` | `examples/tasks/task-source-item*.yaml` | implemented | Immutable upload, repository-Markdown, or inline source metadata with sanitized preview and stale/digest state. |
| Execution and quality | `task-runner-selection.md` | `task-runner-selection` | `examples/tasks/task-runner-selection*.yaml` | implemented | Approved route/readiness projection with requested/effective model and effort metadata and no raw provider flags. |
| Execution and quality | `task-review.md` | `task-review` | `examples/tasks/task-review*.yaml` | implemented | Bounded changed-file and selected-file diff read model with sanitization, truncation, freshness, and durable lineage. |
...
| Core packets and profiles | `execution-profile.md` | `execution-profile` | `examples/execution/execution-profile*.yaml` | implemented | Read-only projection derived from portable route defaults and canonical route/adapter registries. |
| Execution and quality | `execution-readiness-report.md` | `execution-readiness-report` | `examples/execution/execution-readiness-report*.yaml` | implemented | Durable credential-free runner/auth/model/capability/policy readiness summary. |
| Execution and quality | `external-run-projection.md` | `external-run-projection` | `examples/reports/external-run-projection*.yaml` | implemented | Generic query-safe ingress for external controller health without private filename or workspace-layout coupling. |
...
| Execution and quality | `integration-report.md` | `integration-report` | `examples/reports/integration-report-*.yaml` | implemented | Parent-owned child-output application, aggregate gates, stale boundary, repair lineage, blockers, and retained integration workspace evidence. |
...
| Execution and quality | `quality-repair-attempt.md` | `quality-repair-attempt` | `examples/reports/quality-repair-attempt*.yaml` | implemented | Immutable attempt lineage with closed reservation/terminal status vocabulary, positive attempt index, workspace ownership, fingerprints, budget, and evidence refs. |
