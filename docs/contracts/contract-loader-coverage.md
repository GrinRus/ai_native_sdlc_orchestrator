...
| Core packets and profiles | `delivery-plan.md` | `delivery-plan` | `examples/packets/delivery-plan-*.yaml` | implemented | Includes closed-set enum checks for `delivery_mode` (`no-write|patch-only|local-branch|fork-first-pr`) and `status` (`ready|blocked`); strict delivery uses optional `preconditions.runtime_harness` gate evidence. |
...
| Execution and quality | `live-e2e-observation-report.md` | `live-e2e-observation-report` | `examples/reports/live-e2e-observation-report*.yaml` | implemented | Factual-only black-box step journal with required per-step plan, controller-state ref, forbidden legacy matrix/quality/verdict fields, and closed-set run-observation statuses. |
| Execution and quality | `live-e2e-run-health-report.md` | `live-e2e-run-health-report` | `examples/reports/live-e2e-run-health-report*.yaml` | implemented | Factual run-health contract for lifecycle, command, controller, diagnostic, evidence, owner/phase/class, and resume/interaction gaps without code/artifact/UI quality judgement. |
| Execution and quality | `live-e2e-quality-assessment-report.md` | `live-e2e-quality-assessment-report` | `examples/reports/live-e2e-quality-assessment-report*.yaml` | implemented | Post-run SWE-agent outcome assessment with required dimensions, evidence strength, normalized findings taxonomy, explicit not-evaluated/weak/strong gap reporting, and a separate all-pass gate for strict local quality closure. |
| Execution and quality | `live-e2e-step-quality-assessment-request.md` | `live-e2e-step-quality-assessment-request` | `examples/reports/live-e2e-step-quality-assessment-request*.yaml` | implemented | Per-step evaluator request with mission class, feature-size policy, requested assessment method, rubric, expected report ref, and public evaluator input refs. |
| Execution and quality | `live-e2e-step-quality-assessment-report.md` | `live-e2e-step-quality-assessment-report` | `examples/reports/live-e2e-step-quality-assessment-report*.yaml` | implemented | Per-step black-box evaluator report with linked request, method, rubric version, product-change evidence-backed rationale, accepted continue/request-repair/retry/block decisions, required quality dimensions, and public evidence refs. |
...
| Operations | `live-e2e-target-catalog.md` | `live-e2e-target-catalog` | `scripts/live-e2e/catalog/targets/*.yaml` | implemented | Required fields plus semantic checks for mission class, feature-size policy, explicit product rubrics, widened budgets, matrix-cell mission linkage, and xlarge/manual-only coverage. |
