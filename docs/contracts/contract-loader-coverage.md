...
| Execution and quality | `runtime-harness-report.md` | `runtime-harness-report` | `examples/reports/runtime-harness-report*.yaml` | implemented | Required fields, closed-set runtime/mission decisions, and nested run-level controller evidence when present. |
| Execution and quality | `live-e2e-observation-report.md` | `live-e2e-observation-report` | `examples/reports/live-e2e-observation-report*.yaml` | implemented | Required fields, required per-step plan, controller-state ref, forbidden legacy matrix fields, and closed-set status checks for black-box step-controller reports. |
