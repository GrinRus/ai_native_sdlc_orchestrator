...
For strict non-`no-write` delivery, `approval_context.runtime_harness` should mirror the delivery plan precondition so downstream review can verify the latest run-level Runtime Harness report, pass decisions, routed decision count, mission-scoped changed paths, and absence of scope violations.
