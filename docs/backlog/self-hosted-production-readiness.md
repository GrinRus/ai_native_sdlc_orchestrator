...
|---|---|---|
| Nested packet/report/event validation is not production-strict. | `W23-S01` | Invalid nested shapes fail loader tests; canonical examples still pass reference integrity. |
| `production-hardened` auth still needs explicit permission fail-closed coverage. | `W23-S02` | API tests for missing, read-only, mutate-only, read+mutate, wrong-project, and redacted denial paths. |
| CLI/API lifecycle behavior still needs a shared service boundary. | `W23-S03` | Dependency scan has no `apps/api -> apps/cli` or `apps/cli -> apps/api` source edge. |
...
As of W22-S01, the matrix records `baseline-covered=60`, `proof-covered=0`, `partial=49`, and `blocked=3`. A story can move to `proof-covered` only when executable evidence proves the story outcome at the required strength.
