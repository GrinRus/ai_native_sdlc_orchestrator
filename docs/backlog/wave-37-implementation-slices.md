...
4. Add regression coverage that a stuck browser/setup install fails closed with readable evidence instead of preventing controller decisions indefinitely.
5. Retry Codex small only far enough to prove the target setup path reaches either a controller decision or a bounded target blocker.
...
4. W35-S05 docs point to W37-S01 as the prerequisite before Codex/Qwen proof retry.
5. No deleted bounded/mock-backed proof profiles are restored.
