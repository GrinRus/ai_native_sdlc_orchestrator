...
| Duplicate refresh before recovery | In the same mobile recovery state, the cockpit heading rendered a full-width `Refresh` button before `One Recommended Action`, making the first in-card action a secondary refresh instead of the recovery path. | Provider/run-health cockpit headings no longer render a duplicate refresh action; refresh remains available as the secondary action inside `One Recommended Action`, after the primary recovery CTA. |
| Repeated details disclosure names | Recovery and quality cards exposed multiple compact disclosures named only `Details` or `DETAILS`, so keyboard and assistive-tech users could not tell which command, path, or value would expand. | Compact value disclosures now keep the visual `Details` label but expose contextual accessible names such as `Show full command: ...` or `Show full path: ...`. |
| Generic CLI command disclosure | The main `Show CLI command` disclosure stayed visually useful, but its accessible name did not say which command would expand, while the same recovery screen had multiple command/path disclosures. | The recommended-action command disclosure now keeps the visual `Show CLI command` label and exposes a contextual accessible name such as `Show recommended CLI command: aor review decide ...`. |
| New-flow escape hatch during recovery | In the failed-verification recovery state, the mobile focus order reached an enabled `New Flow` control before the primary recovery CTA, making a fresh flow look like a valid way around the unresolved repair. | Blocking run-health gates now disable `New Flow`, expose a contextual disabled reason such as `Resolve the current Recovery Path before starting a new flow.`, and guard the start-new-flow handler so programmatic triggers return the operator to the matching workbench. |
