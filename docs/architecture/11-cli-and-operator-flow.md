...
- `--smoke --open false --json` starts the server, checks `/`, `/app-config.json`, `GET /api/projects`, `GET /api/projects/:projectId/state`, Task Workspace, New task, Prepare task, project-switcher, and retired-renderer-absence markers, prints JSON, then exits;
...
- the packaged UI prepares intent-backed Tasks and submits only Task or lifecycle actions published by the control plane; orchestration remains owned by runtime command handlers.
