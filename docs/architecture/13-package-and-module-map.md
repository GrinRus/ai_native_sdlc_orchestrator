...
- `apps/web` — optional React/Vite Task Workspace source; the npm package serves the built `dist` assets through `aor app`, and app smoke validates the real SPA/config/project-index/state route path, current Task controls, project switcher, and absence of retired renderer markers without a public static snapshot module export
