...
| Connected web | Optional web console launched locally or attached to a control-plane API | same bearer principals as the detached API when the API is production-hardened | `aor app --project-ref <repo>`; `aor app --smoke --open false --json`; `aor ui attach --project-ref <repo> --control-plane <url>` | Web is detachable and must call control-plane read/mutation surfaces. CLI/API/headless operation remains valid without web. |
| npm alpha install | Installed `@grinrus/aor` CLI plus packaged local SPA | npm registry access; runner credentials only for explicit live external-runner workflows | `npm install -g @grinrus/aor@0.1.0-alpha.3`; `aor --help`; `aor doctor --project-ref <repo> --json`; `aor onboard --project-ref <repo> --json`; `aor app --project-ref <repo>` | Public alpha package smoke path proves help, doctor, onboarding, packaged local app smoke, and no surprise writes outside `.aor/`. It does not prove hosted SaaS, Docker/GHCR, SSO, or unattended production automation. |
