...
| Connected web | Optional web console attached to control-plane API | same bearer principals as the detached API when the API is production-hardened | `aor app --help`; `aor ui attach --project-ref <repo> --control-plane <url>` | Web is detachable and must call control-plane read/mutation surfaces. CLI/API/headless operation remains valid without web. |
| npm alpha install | Installed `@grinrus/aor` CLI | npm registry access; runner credentials only for explicit live external-runner workflows | `npm install -g @grinrus/aor@0.1.0-alpha.3`; `aor --help`; `aor doctor --project-ref <repo> --json`; `aor onboard --project-ref <repo> --json` | Public alpha package smoke path proves help, doctor, onboarding, and optional app guidance. It does not prove hosted SaaS, Docker/GHCR, SSO, or unattended production automation. |
