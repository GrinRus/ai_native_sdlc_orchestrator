...
9. For operator-initiated runtime work, use `aor request create/run/status`. Ask AOR in the Task Workspace submits and runs a durable `no-write` request; unfinished requests can be resumed in the Task UI or through the request CLI/API. Keep `delivery-mode=no-write` unless proposal patches are explicitly scoped with allowed paths.
