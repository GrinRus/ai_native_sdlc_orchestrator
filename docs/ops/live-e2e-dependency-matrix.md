...
- required matrix coverage closes only for `run_tier=acceptance` or `run_tier=production-proof` when `live-e2e-run-health-report.overall_status=pass`; medium+ product acceptance additionally requires accepted step-quality reports and final all-pass outcome quality.
