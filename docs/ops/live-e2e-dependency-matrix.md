...
- required matrix coverage closes only for `run_tier=acceptance` or `run_tier=production-proof` when `live-e2e-run-health-report.overall_status=pass`; weak or failing outcome-quality signals remain visible in the separate quality assessment report.
