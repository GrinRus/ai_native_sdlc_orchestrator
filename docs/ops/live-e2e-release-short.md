...
- strict non-`no-write` delivery preserves `approval_context.runtime_harness.status=pass` with Runtime Harness execution evidence and run-level evidence when present;
