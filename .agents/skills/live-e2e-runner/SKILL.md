...
13. After the runner completes, inspect `live_e2e_run_health_report_file` before discussing outcome quality. Read `diagnostic_health` separately from primary run completion: optional diagnostic timeouts/failures are factual run warnings or failures, not outcome-quality verdicts. Separate run-health failures (`owner`, `phase`, `class`, command/controller/provider/target/environment/operator/evidence gaps) from code, artifact, delivery, UI/UX, accessibility, and traceability quality.
