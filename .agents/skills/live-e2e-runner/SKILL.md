...
11. For guided profiles, inspect `frontend_interactions[]` and the linked web smoke evidence. AOR operator browser/UI evidence must be tied to the relevant step observation. When the profile requires `browser-task-proof`, missing proof or a non-passing guided task outcome is a factual run-health `ui_validation` blocker; do not treat deterministic `aor app --smoke` refs as sufficient UX proof.
