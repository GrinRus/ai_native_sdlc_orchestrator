...
export function validateLiveE2eCatalogReferences(options = {}) {
  ...
  /** @type {Map<string, { source: string, liveBaseline: boolean, runtimeMode: string | null, runtimeCommand: string | null }>} */
    ...
    for (const filePath of collectYamlFiles(providerRoot).sort()) {
      ...
      issues.push(
        referenceIssue({
          ...
          field: "primary_adapter",
          reference: adapterRef,
          expected: "adapter with execution.live_baseline=true, runtime_mode=external-process, and external_runtime.command",
          actual: adapterProfile
            ? `live_baseline=${String(adapterProfile.liveBaseline)}, runtime_mode=${adapterProfile.runtimeMode ?? "missing"}, command=${adapterProfile.runtimeCommand ?? "missing"}`
            : "missing adapter profile",
          message: `Required live E2E provider variant references adapter '${adapterRef}' without a live-runnable external runtime.`,
        }),
      );
    }
  }
