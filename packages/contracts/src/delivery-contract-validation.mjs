...
export function validateIntegrationReportContract(document, source) {
  ...
  sourceAttempts.forEach((attempt, index) => {
    ...
    if (attempt.output_kind === "commit" && !/^[0-9a-f]{40}$/u.test(String(attempt.commit_sha ?? ""))) add(issues, source, "field_format_invalid", `source_attempts[${index}].commit_sha`, "40-character Git SHA", String(attempt.commit_sha), "Commit integration outputs must bind to an exact commit object.");
  });
