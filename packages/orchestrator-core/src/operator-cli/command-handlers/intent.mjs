...
function connectProject({ flags, workspace, outputState }) {
...
function prepareTask({ flags, cwd, workspace, projectId, outputState }) {
...
function startTask({ flags, workspace, projectId, outputState }) {
...
function exportTaskEvidence({ flags, workspace, projectId, outputState }) {
...
export function handleIntentCommand({ command, flags, cwd, outputState }) {
  ...
  if (command === "project connect") {
    connectProject({ flags, workspace, outputState });
  } else {
    ...
    } else if (command === "task prepare") {
      prepareTask({ flags, cwd, workspace, projectId, outputState });
    } else if (command === "task start") {
      startTask({ flags, workspace, projectId, outputState });
    } else {
      exportTaskEvidence({ flags, workspace, projectId, outputState });
    }
  }
  return true;
}
