const ROUTES = Object.freeze([
  {
    id: "project-index",
    pattern: /^\/api\/projects$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Project index route supports only GET.",
    params: [],
    kind: "read",
  },
  {
    id: "project-actions",
    pattern: /^\/api\/projects\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Project action route supports only POST.",
    params: [],
    kind: "mutation",
  },
  {
    id: "project-connection-job",
    pattern: /^\/api\/project-connection-jobs\/([^/]+)$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Project connection job route supports only GET.",
    params: ["jobId"],
    kind: "read",
  },
  {
    id: "folder-picker-actions",
    pattern: /^\/api\/workspace\/folder-picker\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Folder picker route supports only POST.",
    params: [],
    kind: "mutation",
  },
  {
    id: "project-topology",
    pattern: /^\/api\/projects\/([^/]+)\/topology$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Project topology route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "intent-submission-list",
    pattern: /^\/api\/projects\/([^/]+)\/intent-submissions$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Intent submission collection supports only GET for reads.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "intent-submission-create",
    pattern: /^\/api\/projects\/([^/]+)\/intent-submissions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Intent submission collection supports POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "intent-submission",
    pattern: /^\/api\/projects\/([^/]+)\/intent-submissions\/([^/]+)$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Intent submission route supports GET.",
    params: ["projectId", "submissionId"],
    kind: "read",
  },
  {
    id: "intent-submission-actions",
    pattern: /^\/api\/projects\/([^/]+)\/intent-submissions\/([^/]+)\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Intent submission action route supports POST.",
    params: ["projectId", "submissionId"],
    kind: "mutation",
  },
  {
    id: "project-topology-validation",
    pattern: /^\/api\/projects\/([^/]+)\/topology\/validation$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Project topology validation route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "project-topology-actions",
    pattern: /^\/api\/projects\/([^/]+)\/topology\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Project topology mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "execution-profile",
    pattern: /^\/api\/projects\/([^/]+)\/execution-profile$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Execution profile route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "execution-profile-actions",
    pattern: /^\/api\/projects\/([^/]+)\/execution-profile\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Execution profile mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "project-state",
    pattern: /^\/api\/projects\/([^/]+)\/state$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "State route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "packets",
    pattern: /^\/api\/projects\/([^/]+)\/packets$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Packet route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "step-results",
    pattern: /^\/api\/projects\/([^/]+)\/step-results$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Step-result route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "quality-artifacts",
    pattern: /^\/api\/projects\/([^/]+)\/quality-artifacts$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Quality route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "delivery-manifests",
    pattern: /^\/api\/projects\/([^/]+)\/delivery-manifests$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Delivery-manifest route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "promotion-decisions",
    pattern: /^\/api\/projects\/([^/]+)\/promotion-decisions$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Promotion-decision route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "strategic-snapshot",
    pattern: /^\/api\/projects\/([^/]+)\/strategic-snapshot$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Strategic-snapshot route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "planner-metrics",
    pattern: /^\/api\/projects\/([^/]+)\/planner-metrics$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Planner-metrics route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "finance-monitoring",
    pattern: /^\/api\/projects\/([^/]+)\/finance-monitoring$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Finance-monitoring route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "next-action-report",
    pattern: /^\/api\/projects\/([^/]+)\/next-action-report$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Next-action report route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "flows",
    pattern: /^\/api\/projects\/([^/]+)\/flows$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow list route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "tasks",
    pattern: /^\/api\/projects\/([^/]+)\/tasks$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Task list route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "task-detail",
    pattern: /^\/api\/projects\/([^/]+)\/tasks\/([^/]+)$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Task detail route supports only GET.",
    params: ["projectId", "taskId"],
    kind: "read",
  },
  {
    id: "task-review",
    pattern: /^\/api\/projects\/([^/]+)\/tasks\/([^/]+)\/review$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Task review route supports only GET.",
    params: ["projectId", "taskId"],
    kind: "read",
  },
  {
    id: "task-actions",
    pattern: /^\/api\/projects\/([^/]+)\/tasks\/([^/]+)\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Task action route supports only POST.",
    params: ["projectId", "taskId"],
    kind: "mutation",
  },
  {
    id: "selected-flow",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/selected$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Selected-flow route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "flow-evidence-graph",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)\/evidence-graph$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow evidence graph route supports only GET.",
    params: ["projectId", "flowId"],
    kind: "read",
  },
  {
    id: "flow-runtime-trace",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)\/runtime-trace$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow runtime trace route supports only GET.",
    params: ["projectId", "flowId"],
    kind: "read",
  },
  {
    id: "flow-attention",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)\/attention$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow attention route supports only GET.",
    params: ["projectId", "flowId"],
    kind: "read",
  },
  {
    id: "flow-plan",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)\/plan$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow plan route supports only GET.",
    params: ["projectId", "flowId"],
    kind: "read",
  },
  {
    id: "flow-plan-progress",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)\/plan\/progress$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow plan progress route supports only GET.",
    params: ["projectId", "flowId"],
    kind: "read",
  },
  {
    id: "flow-detail",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Flow detail route supports only GET.",
    params: ["projectId", "flowId"],
    kind: "read",
  },
  {
    id: "operator-requests",
    pattern: /^\/api\/projects\/([^/]+)\/operator-requests$/u,
    method: "GET",
    allow: "GET, POST",
    permission: "read",
    methodMessage: "Operator-request list route supports GET for reads and POST for creation.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "multirepo-coordination",
    pattern: /^\/api\/projects\/([^/]+)\/multirepo-coordination$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Multirepo coordination route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "compiler-revisions",
    pattern: /^\/api\/projects\/([^/]+)\/compiler-revisions$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Compiler-revision route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "runs",
    pattern: /^\/api\/projects\/([^/]+)\/runs$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Run route supports only GET.",
    params: ["projectId"],
    kind: "read",
  },
  {
    id: "event-history",
    pattern: /^\/api\/projects\/([^/]+)\/runs\/([^/]+)\/events\/history$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Event-history route supports only GET.",
    params: ["projectId", "runId"],
    kind: "read",
  },
  {
    id: "policy-history",
    pattern: /^\/api\/projects\/([^/]+)\/runs\/([^/]+)\/policy-history$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Policy-history route supports only GET.",
    params: ["projectId", "runId"],
    kind: "read",
  },
  {
    id: "run-events",
    pattern: /^\/api\/projects\/([^/]+)\/runs\/([^/]+)\/events$/u,
    method: "GET",
    allow: "GET",
    permission: "read",
    methodMessage: "Run-event stream route supports only GET.",
    params: ["projectId", "runId"],
    kind: "stream",
  },
  {
    id: "run-control-actions",
    pattern: /^\/api\/projects\/([^/]+)\/run-control\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Run-control mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "flow-plan-actions",
    pattern: /^\/api\/projects\/([^/]+)\/flows\/([^/]+)\/plan\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Flow plan action route supports only POST.",
    params: ["projectId", "flowId"],
    kind: "mutation",
  },
  {
    id: "operator-request-create",
    pattern: /^\/api\/projects\/([^/]+)\/operator-requests$/u,
    method: "POST",
    allow: "GET, POST",
    permission: "mutate",
    methodMessage: "Operator-request creation route supports GET for reads and POST for creation.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "operator-request-actions",
    pattern: /^\/api\/projects\/([^/]+)\/operator-requests\/([^/]+)\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Operator-request action route supports only POST.",
    params: ["projectId", "requestId"],
    kind: "mutation",
  },
  {
    id: "ui-lifecycle-actions",
    pattern: /^\/api\/projects\/([^/]+)\/ui-lifecycle\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "UI lifecycle mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "lifecycle-command-actions",
    pattern: /^\/api\/projects\/([^/]+)\/lifecycle-command\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Lifecycle command mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "quality-repair-actions",
    pattern: /^\/api\/projects\/([^/]+)\/quality-repair\/actions$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Quality-repair mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
  {
    id: "interaction-answers",
    pattern: /^\/api\/projects\/([^/]+)\/interactions\/answers$/u,
    method: "POST",
    allow: "POST",
    permission: "mutate",
    methodMessage: "Interaction answer mutation route supports only POST.",
    params: ["projectId"],
    kind: "mutation",
  },
]);

const ROUTE_OPENAPI_PATHS = Object.freeze({
  "project-index": "/api/projects",
  "project-actions": "/api/projects/actions",
  "project-connection-job": "/api/project-connection-jobs/{jobId}",
  "folder-picker-actions": "/api/workspace/folder-picker/actions",
  "project-topology": "/api/projects/{projectId}/topology",
  "intent-submission-create": "/api/projects/{projectId}/intent-submissions",
  "intent-submission-list": "/api/projects/{projectId}/intent-submissions",
  "intent-submission": "/api/projects/{projectId}/intent-submissions/{submissionId}",
  "intent-submission-actions": "/api/projects/{projectId}/intent-submissions/{submissionId}/actions",
  "project-topology-validation": "/api/projects/{projectId}/topology/validation",
  "project-topology-actions": "/api/projects/{projectId}/topology/actions",
  "execution-profile": "/api/projects/{projectId}/execution-profile",
  "execution-profile-actions": "/api/projects/{projectId}/execution-profile/actions",
  "project-state": "/api/projects/{projectId}/state",
  packets: "/api/projects/{projectId}/packets",
  "step-results": "/api/projects/{projectId}/step-results",
  "quality-artifacts": "/api/projects/{projectId}/quality-artifacts",
  "delivery-manifests": "/api/projects/{projectId}/delivery-manifests",
  "promotion-decisions": "/api/projects/{projectId}/promotion-decisions",
  "strategic-snapshot": "/api/projects/{projectId}/strategic-snapshot",
  "planner-metrics": "/api/projects/{projectId}/planner-metrics",
  "finance-monitoring": "/api/projects/{projectId}/finance-monitoring",
  "next-action-report": "/api/projects/{projectId}/next-action-report",
  flows: "/api/projects/{projectId}/flows",
  tasks: "/api/projects/{projectId}/tasks",
  "task-detail": "/api/projects/{projectId}/tasks/{taskId}",
  "task-review": "/api/projects/{projectId}/tasks/{taskId}/review",
  "task-actions": "/api/projects/{projectId}/tasks/{taskId}/actions",
  "selected-flow": "/api/projects/{projectId}/flows/selected",
  "flow-evidence-graph": "/api/projects/{projectId}/flows/{flowId}/evidence-graph",
  "flow-runtime-trace": "/api/projects/{projectId}/flows/{flowId}/runtime-trace",
  "flow-attention": "/api/projects/{projectId}/flows/{flowId}/attention",
  "flow-plan": "/api/projects/{projectId}/flows/{flowId}/plan",
  "flow-plan-progress": "/api/projects/{projectId}/flows/{flowId}/plan/progress",
  "flow-detail": "/api/projects/{projectId}/flows/{flowId}",
  "operator-requests": "/api/projects/{projectId}/operator-requests",
  "multirepo-coordination": "/api/projects/{projectId}/multirepo-coordination",
  "compiler-revisions": "/api/projects/{projectId}/compiler-revisions",
  runs: "/api/projects/{projectId}/runs",
  "event-history": "/api/projects/{projectId}/runs/{runId}/events/history",
  "policy-history": "/api/projects/{projectId}/runs/{runId}/policy-history",
  "run-events": "/api/projects/{projectId}/runs/{runId}/events",
  "run-control-actions": "/api/projects/{projectId}/run-control/actions",
  "flow-plan-actions": "/api/projects/{projectId}/flows/{flowId}/plan/actions",
  "operator-request-create": "/api/projects/{projectId}/operator-requests",
  "operator-request-actions": "/api/projects/{projectId}/operator-requests/{requestId}/actions",
  "ui-lifecycle-actions": "/api/projects/{projectId}/ui-lifecycle/actions",
  "lifecycle-command-actions": "/api/projects/{projectId}/lifecycle-command/actions",
  "quality-repair-actions": "/api/projects/{projectId}/quality-repair/actions",
  "interaction-answers": "/api/projects/{projectId}/interactions/answers",
});

export function listControlPlaneRoutes() {
  return ROUTES.map((route) => ({
    id: route.id,
    method: route.method,
    path: ROUTE_OPENAPI_PATHS[route.id] ?? null,
    permission: route.permission,
    kind: route.kind,
    params: [...route.params],
  }));
}

/**
 * @param {string} pathname
 * @param {string} [method]
 * @returns {{ route: (typeof ROUTES)[number], params: Record<string, string> } | null}
 */
export function matchControlPlaneRoute(pathname, method) {
  /** @type {{ route: (typeof ROUTES)[number], params: Record<string, string> } | null} */
  let methodMismatch = null;
  for (const route of ROUTES) {
    const match = route.pattern.exec(pathname);
    if (!match) {
      continue;
    }
    const params = {};
    route.params.forEach((name, index) => {
      params[name] = decodeURIComponent(match[index + 1]);
    });
    const matched = { route, params };
    if (!method || route.method === method) {
      return matched;
    }
    methodMismatch ??= matched;
  }
  return methodMismatch;
}
