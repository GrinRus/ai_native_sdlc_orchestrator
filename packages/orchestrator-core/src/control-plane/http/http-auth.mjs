 ...
 * @returns {boolean}
 */
function isHttpWhitespace(character) {
...
function extractBearerToken(value) {
  ...
  if (headerValue.length <= "Bearer ".length || headerValue.slice(0, 6).toLowerCase() !== "bearer") {
    return null;
  }
  let tokenStart = 6;
  if (!isHttpWhitespace(headerValue[tokenStart])) {
    return null;
  }
  while (tokenStart < headerValue.length && isHttpWhitespace(headerValue[tokenStart])) {
    tokenStart += 1;
  }
  return asString(headerValue.slice(tokenStart));
}
